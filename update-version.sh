...
# Update *.spec
find dyndns syncer server powerdns_sync -name "*.spec" -type f | while read f; do
...
# Update */Makefile.PL
find dyndns syncer server zonefileimporter powerdns_sync -name "Makefile.PL" | while read f; do
	version_subs="%%s/'VERSION' => '.*',/'VERSION' => '$version',/"
	ed_script=`printf "$version_subs\nw\nq\n"`
	echo "$ed_script" | ed "$f"
done
...
# Update */control
find dyndns syncer server zonefileimporter powerdns_sync -name "control" | while read f; do
...
# Update */changelog
find dyndns syncer server zonefileimporter powerdns_sync -name "changelog" | while read f; do
